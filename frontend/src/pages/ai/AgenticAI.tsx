...
function AgentDetailPage({ service }: { service: AIServiceDetail }) {
  return (
    <>
      {/* Hero */}
      ...
      <section className="section-sm">
        <div className="container" style={{ maxWidth: 700 }}>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="glass-card p-12 text-center">
            <h2 className="t-h2 mb-4">Ready to deploy {service.title.toLowerCase()}?</h2>
            <p className="t-body mx-auto mb-8" style={{ maxWidth: 480, color: 'var(--text-secondary)' }}>
              Automate complex workflows and scale your operations with intelligent AI agents.
            </p>
            <div className="flex flex-wrap items-center justify-center gap-4">
              ...
              <Link to="/ai/agentic-ai" className="btn-outline">Explore All Agentic AI</Link>
            </div>
          </motion.div>
        </div>
      </section>
    ...
    <>
      ...
      <section className="relative overflow-hidden" style={{ paddingTop: 120, paddingBottom: 80 }}>
        <div className="container relative" style={{ zIndex: 2 }}>
          ...
          <motion.p initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.16 }} className="t-body text-center mx-auto mt-5" style={{ maxWidth: 560, color: 'var(--text-secondary)' }}>
            Deploy intelligent agents that reason through complex tasks, use tools, make decisions, and adapt their strategies to achieve your business objectives autonomously.
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.24 }} className="flex flex-wrap items-center justify-center gap-4 mt-10">
            <Link to="/contact" className="btn-primary">
              Deploy AI Agents
      ...
      <section className="section-sm">
        <div className="container" style={{ maxWidth: 900 }}>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-12">
            <h2 className="t-h2 mb-4">What is Agentic AI?</h2>
            <p className="t-body mx-auto" style={{ maxWidth: 600, color: 'var(--text-secondary)' }}>
              Agentic AI refers to autonomous intelligent systems that can make decisions, take actions, and learn from outcomes without constant human intervention.
            </p>
          </motion.div>
          <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-40px' }} className="grid md:grid-cols-3 gap-6">
            {[
              { title: 'Goal-Oriented', desc: 'Agents work towards defined objectives autonomously, breaking complex goals into actionable steps.', icon: 'M13 10V3L4 14h7v7l9-11h-7z' },
              { title: 'Self-Learning', desc: 'Continuously improve from experience, feedback, and changing conditions without manual retraining.', icon: 'M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15' },
              { title: 'Action-Taking', desc: 'Execute tasks, call APIs, query databases, and interact with systems to complete real work.', icon: 'M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z' },
            ].map((item, i) => (
              <motion.div key={i} custom={i} variants={fadeUp} className="glass-card p-7 text-center">
      ...
      <section className="section">
        <div className="container">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-14">
            <h2 className="t-h2">Agentic AI Services</h2>
            <p className="t-body mx-auto mt-4" style={{ maxWidth: 500, color: 'var(--text-secondary)' }}>Intelligent automation solutions for modern businesses</p>
          </motion.div>
          <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-40px' }} className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {AGENTIC_AI_SERVICES.map((svc, i) => (
              <motion.div key={svc.id} custom={i} variants={fadeUp}>
                <Link to={`/ai/agentic-ai/${svc.id}`} className="block glass-service-card group">
      ...
      <section className="section">
        <div className="container">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-14">
            <h2 className="t-h2">Why Agentic AI?</h2>
            <p className="t-body mx-auto mt-4" style={{ maxWidth: 500, color: 'var(--text-secondary)' }}>Transform your business with autonomous intelligence</p>
          </motion.div>
      ...
      <section className="section-sm">
        <div className="container" style={{ maxWidth: 700 }}>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="glass-card p-12 text-center">
            <h2 className="t-h2 mb-4">Ready to Deploy AI Agents?</h2>
            <p className="t-body mx-auto mb-8" style={{ maxWidth: 480, color: 'var(--text-secondary)' }}>
...
export default function AgenticAI() {
  ...
  const selected = slug ? AGENTIC_AI_SERVICES.find((s) => s.id === slug) : undefined;
  return selected ? <AgentDetailPage service={selected} /> : <AgentListPage />;
}
